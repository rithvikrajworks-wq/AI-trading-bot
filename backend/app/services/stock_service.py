...
def _ta_single_series(result: Any) -> pd.Series:
    """Coerce pandas_ta output to a single Series for column assignment."""
...
class StockService:
    ...
    async def get_raw_analysis_data(self, ticker: str) -> Dict[str, Any]:
        ...
        df_daily['RSI'] = _ta_single_series(df_daily.ta.rsi(length=14))
        df_daily['EMA_20'] = _ta_single_series(df_daily.ta.ema(length=20))
        df_daily['EMA_50'] = _ta_single_series(df_daily.ta.ema(length=50))
        ...
        df_daily['ATR'] = _ta_single_series(df_daily.ta.atr(length=14))
    ...
    async def analyze_ticker(self, ticker: str) -> Dict[str, Any]:
        ...
        # Breakout probability is now generated by Gemini AI; deterministic calculation removed.

        pros_candidates = [
            ai_response.bull_case,
            ai_response.investment_thesis,
            ai_response.trend_analysis,
            ai_response.momentum_analysis,
        ]
        pros = [
            text
            for text in (str(value).strip() for value in pros_candidates if value)
            if text and not text.startswith("No ")
        ] or ["No pros provided."]

        cons_candidates = [ai_response.bear_case] + list(ai_response.warnings or [])
        cons = [
            text
            for text in (str(value).strip() for value in cons_candidates if value)
            if text and not text.startswith("No ")
        ] or ["No cons provided."]
        ...
        return {
            ...
            "breakout_probability": ai_response.breakout_probability,
